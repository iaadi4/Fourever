...
const panels = [
  { label: 'कमल तालाब से तोड़ाई', sublabel: 'Makhana harvested from lotus ponds', Component: MithilaHarvestScene, alt: 'Mithila art depicting makhana harvesting from lotus ponds in Bihar' },
  { label: 'लकड़ी की आँच पर भुनाई', sublabel: 'Makhana roasted over wood fire', Component: MithilaRoastScene, alt: 'Traditional wood-fire roasting of makhana foxnuts' },
  { label: 'हाथों से सजाकर पैकिंग', sublabel: 'Makhana hand-packed with love', Component: MithilaPackScene, alt: 'Hand-packing of premium makhana with Mithila art packaging' },
];
...
export function Heritage() {
  ...
  return (
    <section id="heritage" aria-label="Makhana Heritage - How Our Foxnuts are Made in Bihar" className="bg-mithila-crimson text-mithila-ivory py-24 lg:py-32 border-y-4 border-mithila-ink overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
        <motion.div className="w-full flex flex-col gap-5" variants={staggerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }}>
          {panels.map(({ label, sublabel, Component, alt }, idx) => (
            <motion.figure key={idx} variants={panelVariants} className="relative bg-mithila-ivory border-4 border-mithila-ink overflow-hidden group">
              <div className="w-full h-4 bg-mithila-ochre border-b-2 border-mithila-ink flex items-center justify-around overflow-hidden" aria-hidden="true">
                {[...Array(12)].map((_, i) => <div key={i} className="w-2 h-2 bg-mithila-ink rotate-45"></div>)}
              </div>
              <div className="w-full aspect-[2/1]" role="img" aria-label={alt}>
                <Component className="w-full h-full" />
              </div>
              <figcaption className="w-full bg-mithila-ink/90 border-t-2 border-mithila-ink px-4 py-2.5 flex items-center gap-3">
        ...
        <motion.div className="flex flex-col justify-center" variants={staggerVariants} initial="hidden" whileInView="visible" viewport={{ once: true }}>
          <motion.div variants={itemVariants} className="mb-10">
            <svg viewBox="0 0 24 24" className="w-8 h-8 text-mithila-gold mb-4" fill="currentColor" aria-hidden="true">
            ...
            <cite className="font-body text-base text-mithila-ivory/60 mt-3 block not-italic">— रामनाथ मांझी, Makhana Farmer, Darbhanga, Bihar</cite>
          </motion.div>

          <motion.div variants={itemVariants} className="relative flex items-center justify-between mb-14" role="img" aria-label="Makhana production process: Harvest, Roast, Pack">
            <svg className="absolute inset-0 w-full h-full pointer-events-none" preserveAspectRatio="none" viewBox="0 0 100 40" aria-hidden="true">
          ...
          <motion.div variants={itemVariants} className="flex flex-wrap gap-3" role="list" aria-label="Fourever Makhana quality certifications">
            {['GI Protected Makhana', 'Women-Led Cooperative', 'Zero Preservatives'].map((text, idx) => (
              <div key={idx} role="listitem" className="flex items-center gap-2 bg-mithila-ink/80 text-mithila-gold px-4 py-2.5 border border-mithila-gold/60 font-accent text-xs lg:text-sm tracking-wider uppercase backdrop-blur-sm hover:bg-mithila-ink hover:border-mithila-gold transition-colors duration-300">
                {text}
              </div>
            ))}
          </motion.div>
